...
function findSessionCookie(setCookieHeader: unknown): string | undefined {
  ...
  return cookies.find(
    (cookie): cookie is string =>
      typeof cookie === "string" && cookie.startsWith("boutique.sid="),
  );
}
...
describe("Login API", () => {
  ...
  it("authenticates an active user and establishes a server-side session", async () => {
    ...
    expect(cookies).toEqual(
      expect.arrayContaining([expect.stringContaining("boutique.sid=")]),
    );
  ...
  it("rejects logout when the CSRF token is missing or incorrect", async () => {
    ...
    expect(sessionResponse.body.csrfToken).toBe(loginResponse.body.csrfToken);
  });

  it("rejects anonymous access to every order endpoint", async () => {
    const responses = await Promise.all([
      request(app).get("/api/orders"),
      request(app).get("/api/orders/1"),
      request(app).post("/api/orders").send({}),
      request(app).patch("/api/orders/1/status").send({}),
    ]);
